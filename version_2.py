...
def print_board(board):
    ...
    for i, row in enumerate(board):
        print(f"\033[90m{i}\033[00m\033[95m {'|' + '|'.join(row) + '|'}\033[00m") # pour couleurs voir : https://www.geeksforgeeks.org/print-colors-python-terminal/
...
def intro():
    ...
    if signe_saisi == "B":
        print("Merci de prendre la porte, on s'amusera très bien sans vous ici :)") # penser à rajouter la posibilité de partir à chaque fois
    ...
    print("Pour quitter la partie, pressez B !")#ajouter ça pour chaques parties
...
def jeu():
    ...
    for _ in range(9):  # pour chaque coups ds la grille
        print_board(board) #permet de garder affiché le tableau à chaque tour lancé
        mvt_joueur(board, joueur_actuel) # permet de bouger son pion à chaque tour lancé
        
        if victoire_oupas(board):
            print_board(board) # continue d'afficher le tbleau
